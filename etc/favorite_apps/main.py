...
def main():
    ...
    stdout: str = execute_command(
        ["/usr/bin/gsettings", "get", "org.gnome.shell", "favorite-apps"]
    )
    current_favorite_apps: List[str] = (
        stdout[1:][:-2].replace("'", "").replace(" ", "").split(",")
    )
    ...
    message: str = execute_command(
        [" dconf", "write", "/org/gnome/shell/favorite-apps", str(favorite_apps)]
    )
